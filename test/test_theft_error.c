...
shrink_pre_set_shrinking(
		const struct theft_hook_shrink_pre_info* info, void* penv)
{
	(void)info;
	struct err_env* env = (struct err_env*)penv;
	assert(env->tag == 'e');
...
bits_shrink(struct theft* t, const void* instance, uint32_t tactic, void* penv,
		void** output)
{
	(void)t;
	assert(penv);
...
hook_run_pre_error(const struct theft_hook_run_pre_info* info, void* penv)
{
	struct err_env* env = (struct err_env*)penv;
	assert(env->tag == 'e');
	(void)info;
...
hook_run_post_error(const struct theft_hook_run_post_info* info, void* penv)
{
	struct err_env* env = (struct err_env*)penv;
	assert(env->tag == 'e');
	(void)info;
...
hook_trial_pre_error(const struct theft_hook_trial_pre_info* info, void* penv)
{
	struct err_env* env = (struct err_env*)penv;
	assert(env->tag == 'e');
	(void)info;
...
hook_shrink_post_error(
		const struct theft_hook_shrink_post_info* info, void* penv)
{
	struct err_env* env = (struct err_env*)penv;
	assert(env->tag == 'e');
	(void)info;
...
hook_counterexample_error(
		const struct theft_hook_counterexample_info* info, void* penv)
{
	struct err_env* env = (struct err_env*)penv;
	assert(env->tag == 'e');
	(void)info;
