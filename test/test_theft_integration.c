...
static void
list_unpack_seed(theft_hash seed, int32_t* lower, uint32_t* upper)
{
...
static enum theft_shrink_res
list_shrink(struct theft* t, const void* instance, uint32_t tactic, void* env,
		void** output)
{
	(void)t;
	list* l = (list*)instance;
...
static enum theft_shrink_res
uint_shrink(struct theft* t, const void* instance, uint32_t tactic, void* env,
		void** output)
{
	(void)t;
	(void)env;
...
halt_after_third_shrink_shrink_pre(
		const struct theft_hook_shrink_pre_info* info, void* venv)
{
		...
		const struct theft_hook_shrink_post_info* info, void* env)
{
	...
	printf("    AFTER: %u, done? %d\n", *pnum,
			info->state == THEFT_SHRINK_POST_DONE_SHRINKING);
...
shrink_all_the_way_shrink_post(
		const struct theft_hook_shrink_post_info* info, void* venv)
{
	struct shrink_test_env* env = (struct shrink_test_env*)venv;
...
static enum theft_hook_trial_pre_res
repeat_once_trial_pre(const struct theft_hook_trial_pre_info* info, void* venv)
{
	(void)info;
...
repeat_first_successful_shrink_then_halt_trial_pre(
		const struct theft_hook_trial_pre_info* info, void* venv)
{
	(void)info;
...
static enum theft_hook_trial_pre_res
halt_if_found_10(const struct theft_hook_trial_pre_info* info, void* venv)
{
	(void)info;
	struct crash_env* env = (struct crash_env*)venv;
	return env->minimum ? THEFT_HOOK_TRIAL_PRE_HALT
