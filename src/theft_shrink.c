...
attempt_to_shrink_arg(struct theft* t, uint8_t arg_i)
{
	...
	for (uint32_t tactic = 0; tactic < THEFT_MAX_TACTICS; tactic++) {
		...
		enum theft_shrink_res sres =
				(use_autoshrink ? theft_autoshrink_shrink(t,
								  as_env,
								  tactic,
								  &candidate,
								  &candidate_bit_pool)
		...
		for (;;) {
			...
			if (stpres == THEFT_HOOK_SHRINK_TRIAL_POST_REPEAT ||
					(stpres == THEFT_HOOK_SHRINK_TRIAL_POST_REPEAT_ONCE &&
							!repeated)) {
				repeated = true;
				continue; // loop and run again
			} else if (stpres == THEFT_HOOK_SHRINK_TRIAL_POST_REPEAT_ONCE &&
					repeated) {
				break;
			} else if (stpres ==
...
shrink_pre_hook(struct theft* t, uint8_t arg_index, void* arg, uint32_t tactic)
{
...
shrink_post_hook(struct theft* t, uint8_t arg_index, void* arg,
		uint32_t tactic, enum theft_shrink_res sres)
{
	if (t->hooks.shrink_post != NULL) {
		enum theft_shrink_post_state state;
		switch (sres) {
		case THEFT_SHRINK_OK:
			state = THEFT_SHRINK_POST_SHRUNK;
			break;
		case THEFT_SHRINK_NO_MORE_TACTICS:
			state = THEFT_SHRINK_POST_DONE_SHRINKING;
			break;
		case THEFT_SHRINK_DEAD_END:
			state = THEFT_SHRINK_POST_SHRINK_FAILED;
			break;
		default:
			assert(false);
