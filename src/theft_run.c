...
static uint8_t infer_arity(const struct theft_run_config* cfg);
...
static enum run_step_res run_step(
		struct theft* t, size_t trial, theft_seed* seed);

static bool copy_propfun_for_arity(
		const struct theft_run_config* cfg, struct prop_info* prop);

static bool check_all_args(uint8_t arity, const struct theft_run_config* cfg,
		bool* all_hashable);
...
static enum all_gen_res gen_all_args(struct theft* t);
...
theft_run_init(const struct theft_run_config* cfg, struct theft** output)
{
	...
	struct fork_info fork = {
			.enable = cfg->fork.enable && THEFT_POLYFILL_HAVE_FORK,
	...
	struct prop_info prop = {
			.name        = cfg->name,
			.arity       = arity,
			.trial_count = cfg->trials == 0 ? THEFT_DEF_TRIALS
							: cfg->trials,
	...
	if (t->hooks.trial_post == theft_hook_trial_post_print_result) {
		...
		t->print_trial_result_env->tag =
				THEFT_PRINT_TRIAL_RESULT_ENV_TAG;
	}
...
theft_run_trials(struct theft* t)
{
	if (t->hooks.run_pre != NULL) {
		...
		enum theft_hook_run_pre_res res =
				t->hooks.run_pre(&hook_info, t->hooks.env);
	...
	if (run_post != NULL) {
		...
		enum theft_hook_run_post_res res =
				run_post(&hook_info, t->hooks.env);
...
run_step(struct theft* t, size_t trial, theft_seed* seed)
{
	...
	if (gen_args_pre != NULL) {
		struct theft_hook_gen_args_pre_info hook_info = {
				...
				.arity        = t->prop.arity};
		enum theft_hook_gen_args_pre_res res =
				gen_args_pre(&hook_info, t->hooks.env);
...
static bool
init_arg_info(struct theft* t, struct trial_info* trial_info)
{
	for (size_t i = 0; i < t->prop.arity; i++) {
		...
		if (ti->autoshrink_config.enable) {
			...
			trial_info->args[i].u.as.env =
					theft_autoshrink_alloc_env(t, i, ti);
...
gen_all_args(struct theft* t)
{
	for (uint8_t i = 0; i < t->prop.arity; i++) {
		...
		if (res == THEFT_RESULT_SKIP) {
			return ALL_GEN_SKIP;
		} else if (res == THEFT_RESULT_ERROR) {
...
static void
free_print_trial_result_env(struct theft* t)
{
	if (t->hooks.trial_post == theft_hook_trial_post_print_result &&
