...
void
theft_hash_init(struct theft_hasher* h)
{
	assert(h);
...
void
theft_hash_sink(struct theft_hasher* h, const uint8_t* data, size_t bytes)
{
	assert(h);
	assert(data);
	if (h == NULL || data == NULL) {
...
theft_hash
theft_hash_done(struct theft_hasher* h)
{
	assert(h);
...
theft_hash
theft_hash_onepass(const uint8_t* data, size_t bytes)
{
	assert(data);
