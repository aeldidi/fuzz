...
void
theft_generic_free_cb(void* instance, void* env)
{
	(void)env;
	free(instance);
}
		...
		const struct theft_hook_trial_post_info*      info)
{
	...
	if (t->print_trial_result_env == env) {
		assert(t->print_trial_result_env->tag ==
			...
			env == t->hooks.env) {
		if (env != NULL &&
				env->tag != THEFT_PRINT_TRIAL_RESULT_ENV_TAG) {
			fprintf(stderr, "\n"
					"WARNING: The *env passed to "
					"trial_print_trial_result is probably "
					"not\n"
					"a `theft_print_trial_result_env` "
	...
	const uint8_t maxcol = (env->max_column == 0 ? THEFT_DEF_MAX_COLUMNS
						     : env->max_column);
...
{
	(void)env;
	return info->failures > 0 ? THEFT_HOOK_TRIAL_PRE_HALT
...
{
	theft_print_trial_result(
			(struct theft_print_trial_result_env*)env, info);
...
void
theft_print_run_pre_info(FILE* f, const struct theft_hook_run_pre_info* info)
{
...
void
theft_print_run_post_info(FILE* f, const struct theft_hook_run_post_info* info)
{
...
void*
theft_hook_get_env(struct theft* t)
{
	return t->hooks.env;
}
