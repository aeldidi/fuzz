...
void
theft_random_set_seed(struct theft* t, uint64_t seed)
{
...
void
theft_random_inject_autoshrink_bit_pool(
		struct theft* t, struct autoshrink_bit_pool* bit_pool)
{
	t->prng.bit_pool = bit_pool;
}

void
theft_random_stop_using_bit_pool(struct theft* t)
{
...
uint64_t
theft_random_bits(struct theft* t, uint8_t bit_count)
{
	assert(bit_count <= 64);
	LOG(4,
...
void
theft_random_bits_bulk(struct theft* t, uint32_t bit_count, uint64_t* buf)
{
	LOG(5, "%s: bit_count %u\n", __func__, bit_count);
	assert(buf);
	if (t->prng.bit_pool) {
		theft_autoshrink_bit_pool_random(
...
double
theft_random_double(struct theft* t)
{
...
uint64_t
theft_random_choice(struct theft* t, uint64_t ceil)
{
	if (ceil < 2) {
		return 0;
	}
