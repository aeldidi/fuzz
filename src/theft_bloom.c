...
struct theft_bloom*
theft_bloom_init(const struct theft_bloom_config* config)
{
...
bool
theft_bloom_mark(struct theft_bloom* b, uint8_t* data, size_t data_size)
{
...
bool
theft_bloom_check(struct theft_bloom* b, uint8_t* data, size_t data_size)
{
	uint64_t hash = theft_hash_onepass(data, data_size);
	LOG(3 - LOG_BLOOM, "%s: overall hash: 0x%016" PRIx64 "\n", __func__,
			hash);
...
void
theft_bloom_free(struct theft_bloom* b)
{
	const size_t top_block_count = (1LLU << b->top_block2);
