...
void
theft_autoshrink_free_env(struct theft* t, struct autoshrink_env* env)
{
	(void)t;
	if (env->bit_pool != NULL) {
		theft_autoshrink_free_bit_pool(t, env->bit_pool);
	}
	free(env);
}
...
static void
lazily_fill_bit_pool(struct theft* t, struct autoshrink_bit_pool* pool,
...
void
theft_autoshrink_free_bit_pool(
		...
		struct autoshrink_bit_pool** output_bit_pool)
{
	...
	if (3 - LOG_AUTOSHRINK <= THEFT_LOG_LEVEL) {
		theft_autoshrink_dump_bit_pool(stdout, orig->bits_filled, orig,
	...
	if (3 - LOG_AUTOSHRINK <= THEFT_LOG_LEVEL) {
		theft_autoshrink_dump_bit_pool(stdout, copy->bits_filled, copy,
				THEFT_AUTOSHRINK_PRINT_ALL);
	}
...
static void
drop_from_bit_pool(struct theft* t, struct autoshrink_env* env,
...
static void
mutate_bit_pool(struct theft* t, struct autoshrink_env* env,
		const struct autoshrink_bit_pool* orig,
...
static bool
choose_and_mutate_request(struct theft* t, struct autoshrink_env* env,
		const struct autoshrink_bit_pool* orig,
		struct autoshrink_bit_pool*       pool)
{
	autoshrink_prng_fun* prng  = get_prng(t, env);
...
void
theft_autoshrink_dump_bit_pool(FILE* f, size_t bit_count,
...
void
theft_autoshrink_print(struct theft* t, FILE* f, struct autoshrink_env* env,
		const void* instance, void* type_env)
{
	...
	if (print_mode == THEFT_AUTOSHRINK_PRINT_DEFAULT) {
		print_mode = (ti->print == NULL ? THEFT_AUTOSHRINK_PRINT_REQUESTS
...
get_prng(struct theft* t, struct autoshrink_env* env)
{
	if (env->prng) {
		return env->prng;
	} else {
...
static bool
should_drop(struct theft* t, struct autoshrink_env* env, size_t request_count)
{
	autoshrink_prng_fun* prng = get_prng(t, env);
...
void
theft_autoshrink_model_set_next(
