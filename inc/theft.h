...
THEFT_PUBLIC
int theft_run(const struct theft_run_config* cfg);
...
THEFT_PUBLIC
int theft_generate(FILE* f, theft_seed seed,
...
THEFT_PUBLIC
void theft_hash_sink(
		struct theft_hasher* h, const uint8_t* data, size_t bytes);

// Finish hashing and get the result. (This also resets the internal hasher
// state.)
THEFT_PUBLIC
...
//
// To use this, add a `struct theft_print_trial_result_env` to the env in the
// `struct theft_run_config`, and call `theft_print_trial_result` with it from
// inside the `trial_post` hook.
//
// When the default `theft_hook_trial_post_print_result` hook is used, the env
...
THEFT_PUBLIC
void theft_print_trial_result(struct theft_print_trial_result_env* print_env,
...
THEFT_PUBLIC
int theft_hook_trial_post_print_result(
...
THEFT_PUBLIC
const char* theft_trial_res_str(int res);

// Return a string name of a run result.
THEFT_PUBLIC
const char* theft_run_res_str(int res);
...
enum theft_builtin_type_info {
	...
	//
	// For example, if the theft_type_info struct's env field is set like
	// this:
	//
	//     uint8_t limit = 64;
...
THEFT_PUBLIC
const struct theft_type_info* theft_get_builtin_type_info(
		enum theft_builtin_type_info type);

// Copy a built-in type info into INFO, so its fields can be modified (e.g.
// setting a limit in info->env).
THEFT_PUBLIC
void theft_copy_builtin_type_info(enum theft_builtin_type_info type,
		struct theft_type_info*                        info);
