...
#define THEFT_RESULT_OK        (0) // No failure
#define THEFT_RESULT_FAIL      (1) // 1 or more failures
#define THEFT_RESULT_SKIP      (2)
#define THEFT_RESULT_DUPLICATE (3) // Skipped because this was already done

#define THEFT_RESULT_ERROR_MEMORY (-1) // Memory allocation failure
#define THEFT_RESULT_ERROR        (-3)
...
typedef int theft_propfun1(struct theft* t, void* arg1);
typedef int theft_propfun2(struct theft* t, void* arg1, void* arg2);
typedef int theft_propfun3(
		struct theft* t, void* arg1, void* arg2, void* arg3);
typedef int theft_propfun4(struct theft* t, void* arg1, void* arg2, void* arg3,
		void* arg4);
typedef int theft_propfun5(struct theft* t, void* arg1, void* arg2, void* arg3,
		void* arg4, void* arg5);
typedef int theft_propfun6(struct theft* t, void* arg1, void* arg2, void* arg3,
		void* arg4, void* arg5, void* arg6);
typedef int theft_propfun7(struct theft* t, void* arg1, void* arg2, void* arg3,
...
// Allocate and return an instance of the type, based on a pseudo-random number
// stream with a known seed. To get random numbers, use
// `theft_random_bits(t, bit_count)` or `theft_random_bits_bulk(t, bit_count,
// buffer)`. This stream of numbers will be deterministic, so if the alloc
// callback is constructed appropriately, an identical instance can be
// constructed later from the same initial seed and environment.
//
// The allocated instance should be written into *output.
//
// If autoshrinking is used, then alloc has an additional requirement: getting
// smaller values from `theft_random_bits` should correspond to simpler
// instances. In particular, if `theft_random_bits` returns 0 forever, alloc
// must generate a minimal instance.
typedef int theft_alloc_cb(struct theft* t, void* env, void** output);

// Free an instance.
typedef void theft_free_cb(void* instance, void* env);

// Hash an instance. Used to skip combinations of arguments which have probably
// already been checked.
typedef theft_hash theft_hash_cb(const void* instance, void* env);
...
// return THEFT_SHRINK_DEAD_END to indicate that the instance cannot be
...
// THEFT_SHRINK_NO_MORE_TACTICS. Otherwise, theft will keep calling the
...
typedef enum theft_shrink_res theft_shrink_cb(struct theft* t,
		const void* instance, uint32_t tactic, void* env,
		void** output);

// Print INSTANCE to output stream F.
// Used for displaying counter-examples. Can be NULL.
typedef void theft_print_cb(FILE* f, const void* instance, void* env);
...
// halt everything, clean up, and return `THEFT_RESULT_ERROR`.
...
typedef enum theft_hook_run_pre_res theft_hook_run_pre_cb(
		const struct theft_hook_run_pre_info* info, void* env);
...
typedef enum theft_hook_run_post_res theft_hook_run_post_cb(
		const struct theft_hook_run_post_info* info, void* env);
...
typedef enum theft_hook_gen_args_pre_res theft_hook_gen_args_pre_cb(
		const struct theft_hook_gen_args_pre_info* info, void* env);
...
typedef enum theft_hook_trial_pre_res theft_hook_trial_pre_cb(
...
#define THEFT_HOOK_FORK_POST_ERROR    (0)
#define THEFT_HOOK_FORK_POST_CONTINUE (1)
typedef int theft_hook_fork_post_cb(
		const struct theft_hook_fork_post_info* info, void* env);
...
// result.
#define THEFT_HOOK_TRIAL_POST_ERROR       (0)
#define THEFT_HOOK_TRIAL_POST_CONTINUE    (1)
#define THEFT_HOOK_TRIAL_POST_REPEAT      (2) // repeat with the same arguments
#define THEFT_HOOK_TRIAL_POST_REPEAT_ONCE (3) // Same as REPEAT, but only once
typedef int theft_hook_trial_post_cb(
		const struct theft_hook_trial_post_info* info, void* env);

// Counter-example hook: called when theft finds a counter-example that causes
// a property test to fail.
//
// By default, this just calls `theft_print_counterexample`, but can be
// overridden to log the counterexample some other way.
...
typedef enum theft_hook_counterexample_res theft_hook_counterexample_cb(
		const struct theft_hook_counterexample_info* info, void* env);
...
typedef enum theft_hook_shrink_pre_res theft_hook_shrink_pre_cb(
		const struct theft_hook_shrink_pre_info* info, void* env);
...
typedef enum theft_hook_shrink_post_res theft_hook_shrink_post_cb(
		const struct theft_hook_shrink_post_info* info, void* env);
...
typedef enum theft_hook_shrink_trial_post_res theft_hook_shrink_trial_post_cb(
		const struct theft_hook_shrink_trial_post_info* info,
		void*                                           env);
...
// Default number of columns after which `theft_print_trial_result` should
// wrap.
...
struct theft_run_config {
	...
	// The number refers to the number of generated arguments, and should
	// match the number of `theft_type_info` structs defined in
	// `.type_info` below. (The fields with different argument counts are
	// ignored.)
	...
	struct {
		...
		int signal;
		// For workers sent a timeout signal, how long should theft
