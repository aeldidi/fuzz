...
#define GREATEST_ASSERT_EQUAL_Tm(MSG, EXP, GOT, TYPE_INFO, UDATA)             \
	do {                                                                  \
		...
		if (!greatest_do_assert_equal_t(                              \
				    EXP, GOT, type_info, UDATA)) {            \
			if (type_info == NULL || type_info->equal == NULL) {  \
				GREATEST_FAILm("type_info->equal callback "   \
					       "missing!");                   \
			} else {                                              \
				GREATEST_FAILm(MSG);                          \
			}                                                     \
...
#define GREATEST_SHUFFLE(ID, SD, BODY)                                        \
	do {                                                                  \
		...
		do {                                                          \
			...
			BODY;                                                 \
			if (!prng->initialized) {                             \
				if (!greatest_prng_init_second_pass(          \
						    ID, SD)) {                \
					break;                                \
				}                                             \
...
#define GREATEST_MAIN_DEFS()                                                             \
	...
	{                                                                                \
		...
		while (name[offset] != '\0') {                                           \
			if (name[offset] == filter[0]) {                                 \
				if (0 == strncmp(&name[offset], filter,                  \
							 filter_len)) {                  \
					return 1;                                        \
				}                                                        \
			}                                                                \
	...
	{                                                                                \
		...
		if (g->name_suffix && (len + 1 < size)) {                                \
			g->name_buf[len] = '_';                                          \
			strncat(&g->name_buf[len + 1], g->name_suffix,                   \
					size - (len + 2));                               \
		}                                                                        \
	}                                                                                \
                                                                                         \
 ...
 * test shuffling state, if applicable, and then call setup hooks. */        \
	...
	{                                                                                \
		...
		match = greatest_name_match(                                             \
					g->name_buf, g->test_filter, 1) &&               \
			!greatest_name_match(                                            \
					g->name_buf, g->test_exclude, 0);                \
	...
	{                                                                                \
		...
		if (!greatest_name_match(suite_name,                                     \
				    greatest_info.suite_filter, 1) ||                    \
	...
	{                                                                                \
		...
		report_suite();                                                          \
                                                                                         ...
                                                                                         \
	static void greatest_run_suite(                                                  \
			greatest_suite_cb* suite_cb, const char* suite_name)             \
	{                                                                                \
	...
	{                                                                                \
		GREATEST_FPRINTF(GREATEST_STDOUT,                                        \
				"Usage: %s [-hlfav] [-s SUITE] [-t TEST] "               \
				"[-x EXCLUDE]\n"                                         \
				"  -h, --help  print this Help\n"                        \
				"  -l          List suites and tests, then "             \
				"exit (dry run)\n"                                       \
				"  -f          Stop runner after first "                 \
				"failure\n"                                              \
				"  -a          Abort on first failure "                  \
				"(implies -f)\n"                                         \
				"  -v          Verbose output\n"                         \
				"  -s SUITE    only run suites containing "              \
				"substring SUITE\n"                                      \
				"  -t TEST     only run tests containing "               \
				"substring TEST\n"                                       \
				"  -x EXCLUDE  exclude tests containing "                \
				"substring EXCLUDE\n",                                   \
				name);                                                   \
	}                                                                                \
                                                                                         \
	...
	{                                                                                \
		greatest_info.flags =                                                    \
				(unsigned char)(greatest_info.flags | flag);             \
	}                                                                                \
                                                                                         \
	void greatest_set_test_suffix(const char* suffix)                                \
	{                                                                                \
	...
	{                                                                                \
		...
		return (size != NULL ? (0 == strncmp((const char*)expd,                  \
							     (const char*)got,           \
							     *size))                     \
				     : (0 == strcmp((const char*)expd,                   \
							     (const char*)got)));        \
	}                                                                                \
                                                                                         \
	...
	{                                                                                \
		...
		return GREATEST_FPRINTF(                                                 \
				GREATEST_STDOUT, "%s", (const char*)t);                  \
	}                                                                                \
                                                                                         \
	...
	{                                                                                \
		...
		for (i = 0; i < env->size; i += line_len) {                              \
			...
			for (line_i = i; line_i < i + line_len; line_i++) {              \
				int m = env->exp[line_i] ==                              \
					env->got[line_i]; /* match? */                   \
	...
	{                                                                                \
		...
		prng->state = seed &                                                     \
			      0x1fffffff; /* only use lower 29 bits... */                \
		prng->a = (4LU * prng->state) + 1; /* to avoid overflow */               \
		prng->c = primes[(seed * 16451) % sizeof(primes) /                       \
				 sizeof(primes[0])];                                     \
		prng->initialized = 1;                                                   \
		return 1;                                                                \
	}                                                                                \
